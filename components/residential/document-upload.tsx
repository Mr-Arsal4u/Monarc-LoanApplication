  ...
  return (
    <FormSection title="Document Upload" description="Upload required documents to complete your application">
      <div className="space-y-4">
        {requiredDocuments.map((doc) => (
          <div key={doc} className="flex items-center justify-between p-4 border border-border rounded-lg">
            ...
            <span className={uploadedDocs.includes(doc) ? "text-primary text-sm" : "text-muted-foreground text-sm"}>
      ...
      <button
        ...
        className="w-full mt-8 py-3 px-4 rounded-lg bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
